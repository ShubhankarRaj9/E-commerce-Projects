...
const Stats = () => {
  ...
  const {totalUsersCount,
    todayRevenue,yesterdayRevenue, totalRevenueAllTime
     ...
     stats.map((stat,index)=>{
        return(
          <div key={index} className={`bg-white p-4 rounded-xl shadow-md ${index !==0 && "flex gap-2 flex-col"}`}>
            ...
            <div className={`text-xl font-semibold ${index!==0 && "text-\[30px\] overflow-y-hidden"}`}>
              {stat.value}
              </div>
              {
                stat.change && (
                  <div className={`text-sm ${stat.change.startsWith("+")?"text-green-500":text-red-500}`}>{stat.change} than last Period. </div>
                )
              }
          </div>
