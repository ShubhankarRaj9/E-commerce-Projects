            ...
            <div className="flex flex-wrap gap-2">
              {
                statusArray.map(status =>{
                  return(
                    ...
                    className={`px-4 py-2 rounded-lg font-medium transition-all captialize ${
                      statusFilter ===status?"gradient-primary text-primary-foreground ":"glass-card hover:glow-on-hover text-foreground"
                    }`}
                    >
                      {status}
                    </button>
...

        {
          filterOrders.length === 0?(
            <div className="text-center glass-panel max-w-md mx-auto">
              ...
              <p className="text-muted-foreground">
                {
                  statusFilter ==="All" ?"You have not placed any order yet." :`No orders with status"${statusFilter}" found.`
                }
              </p>
              </div>
          ):(
            <div className="space-y-6">
              {
                filterOrders.map(order => {
                  return (
                    <div key={order.id} className="glass-card p-6">
                      <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-6 space-y-4 md:space-y-0">
                        <div className="">
                          <h3 className="text-lg font-semibold font-foreground mb-1">Orders #{order.id}</h3>
                          <p className="text-muted-foreground">
                            Placed on{""}
                            {
                                  ...
                                  order?.order_items?.map(item =>{
                                    ...
                                    <div key={item.product_id} className="flex items-center space-x-4 p-4 bg-secondary/50 rounded-lg">
                                      ...
                                      <div className="flex-1 min-w-0">
                                        ...
                                        <p className="text-sm text-mutes-foreground">Quantity{item.quantity}</p>
                                        </div>
                                        <div className="text-right">
                  ...
                  {order.status === "Delivered" && (
                    <>
                      <Link to={`/product/""`}className="px-4 py-2 glass-card hover:glow-on-hover animate-smooth text-sm">
                        Write Review 
                      </Link>
                      <button className="px-4 py-2 glass-card hover:glow-on-hover animate-smooth text-sm">
                        Reorder
                ...
                <form onSubmit={handleSubmit} className="glass-panel">
                  ...
                  <div className="mb-6">
                  <label className="block text-sm font-medium text-foreground mb-2"> Card Details *
                  </label>
               ...
               <button   type="submit" disabled={!razorpay || isProcessing} className="flex justify-center items-center gap-2 w-full py-3 gradient-primary text-primary-foreground rounded-lg hover:glow-on-hover animate-smooth font-semibold" > {isProcessing ? (
               <> <div className={`w-5 h-5 border-2 border-white border-t-transparent rounded-full    animate-spin`} />  <span className="text-white">Payment Processing ...</span>
               </> ) : ( "Complete Payment" )}
                </button>
           </form>
      </div> )})  } </div> ) } </div></div>
