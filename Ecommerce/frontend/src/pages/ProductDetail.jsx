...
import { Star, ShoppingCart, Heart, Share2, Plus, Minus, Loader,CircleDollarSign } from 'lucide-react';
import { useDispatch, useSelector } from 'react-redux';
import ReviewsContainer from '../components/Products/ReviewContainer';
...
const ProductDetail = () => {
  ...
  const handleCopyURL = () =>{
    ...
    navigator.clipboard.writeText(currentUrl).then(()=>{toast.success("URL copied",currentUrl);
    }).catch((err) => {
  ...
  const handleBuy = () => {
    ...
    toast.error("Please select a valid quantity before buying");
    return;
    }
     dispatch(addToCart({product,quantity}));
    toast.success(`${product.name} added to cart. Redirecting to payment 💳`);
     navigateTo("/payment");
  }
  useEffect(()=>{
        ...
        <div className='grid grid-cols-1 lg:grid-cols-2 gap-12 mb-16'>
                ...
                product.images && product?.images.map((image,idx)=>{
                  return (
                    ...
                    className={`w-20 h-20 rounded-lg overflow-hidden border-2 transition-all ${
                      ...
                      <img src={image?.url} alt={`${product.title} ${idx+1}`}
            ...
            <div className="mb-4">
              <div className='flex space-x-2 mb-4'>

                {
                  ...
                  product.ratings > 4.5 && (
                    <span className="px-2 py-1 bg-gradient-to-right  from bg-yellow-400 to-rose-500
          ...
          <div className='glass-panel'>
            <div className='flex border-b border-[hsla(var(--glass-border))]'>
              {
                ["descriptive", "reviews"].map((tab)=>{
                  return (
                    <button key={tab} onClick={()=>setActiveTab(tab)} className={`px-6 py-4 font-medium capitalize transition-all ${activeTab === tab ? "text-primary border-b-2 border-primary": "text-muted-foreground hover:text-foreground"}`}>
                      {tab}
                    </button>
                  )
                })
                ...
                activeTab ==="description" && (
                  <div>
                    <h3 className='text-xl font-semibold text-foreground mb-4'>Product Description</h3>
                     <p className='text-muted-foreground loaing-relaxed'
