...
const CartSidebar = () => {
  ...
  total = cart.reduce((sum, item) => {
    if (!item || !item.product || item.product.price == null) return sum;
    const price = item?.product?.price ?? 0;      // fallback to 0 if price is missing
    const qty = typeof item?.quantity === 'number' ? item.quantity : 1; // fallback to 1
        ...
        <div className="p-6">{
          cart && cart.length === 0 ? (
            <div className="text-center py-12">
              ...
              className="inline-block mt-4 px-6 py-2 gradient-primary text-primary-foreground rounded-lg hover:glow-on-hover animate-smooth">Browse 
              Products</Link>
              </div>
          ):(<>
          {/* CART ITEMS */}
          <div className="space-y-4 mb-6">
            {
              cart && cart.map(item => {
                return(
                  <div key={item.product._id} className="glass-card p-4">
                      <div className="flex items-center space-x-4">
                        ...
                        onClick={()=>{updateQuantity(item.product._id,item.quantity-1);}} className="p-1 rounded glass-card hover:glow-on-hover animate-smooth">
                          <Minus className="w-4 h-4"/>
                        </button>
                        <span className="w-8 text-center font-semibold">
                          {item.quantity??0}
                        </span>
                        <button onClick={()=>{updateQuantity(item.product._id,item.quantity+1);}} className="p-1 rounded glass-card hover:glow-on-hover animate-smooth">
                          <Plus className="w-4 h-4 "/>
                        </button>
                        
                        <button onClick={()=>{dispatch(removeFromCart(item.product._id))}} className="p-1 rounded glass-card hover:glow-on-hover animate-smooth ml-2 text-destructive">
          ...
          <div className="border-t border-[hsla(var(--glass-border))] pt-4">
            <div className="flex justify-between items-center mb-4">
              <span className="text-lg font-semibold">Total:</span><span className="text-xl font-bold text-primary">${total.toFixed(2)}</span>
            </div>
