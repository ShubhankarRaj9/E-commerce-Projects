...
const LoginModel = () => {
  ...
  const { authUser,isSigningUp,isRequestingForToken,isloggingIn, } = useSelector(state => state.auth);
  const {isAuthPopupOpen} = useSelector(state => state.popup);

  const [mode,setMode] = useState("signin") //signin|signUp|forget|reset
  ...
  // fetch reset password details with reset mode
  useEffect(() => {
  ...
  const handleSubmit = (e) => {
    ...
    if(mode === "forgot"){
      dispatch(forgotPassword({email:formData.email})).then(()=> {dispatch(toggleAuthPopup()); setMode("signin");});return;}

    if(mode === "reset"){
      const token = location.pathname.split("/").pop();
      dispatch(resetPassword({token,password:formData.password,confirmPassword:formData.confirmPassword}));return;}
         ...
         <X className="w-5 h-5 text-primary"/>
          ...
          <form onSubmit={handleSubmit}className="space-y-4">
            ...
            { mode ==="signup" && (
                ...
                {/* Email always visible except mode */}
            { mode !=="reset" && (
                <div className="relative">
                  ...
                  className={`w-full py-3 gradient-primary flex justify-center items-center gap-2 text-primary-foreground rounded-lg font-semibold animate-smooth ${isLoading?"opacity-70 cursor-not-allowed":"hover:glow-on-hover"}`}>
                      {
                        ...
                        <>
                            ...
                            <span>{ mode === "reset" ? "Resetting Password...":mode ==="signup"?"Signing up...":mode==="forgot"?"Requesting for email...":"Signing In"
                            }
                            {" "} </span>
                        </>
                        ):
                        ...
                        {/* Mode Toggle */}
             {["signin","signup"].includes(mode) && (<div
