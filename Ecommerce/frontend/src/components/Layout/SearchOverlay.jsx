...
const SearchOverlay = () => {
  ...
  {/* Glass Background */}
    <div className="absolute inset-0 backdrop-blur-md bf-[hsla(var(--glass-bg))]">
      {/* {Search Container} */}
          ...
          <div className="flex items-center justify-between mb-6">
           ...
           <button onClick={() => dispatch(toggleSearchBar())} className="p-2 rounded-lg glass-card hover:glow-on-hover animate-smooth">
            ...
            <div className="relative">
              ...
              <Search className="text-shadow-white"/>
              </button>
              <input type="text" placeholder="Search for products..." value={searchQuery} onChange={(e)=>setSearchQuery(e.target.value)} onKeyDown={(e) => e.key === "Enter" && handleSearch()}
              className="w-full pl-12 pr-4 py-4 bg-secondary border border-border rounded-lg focus:outline-none text-foreground placeholder-muted-foreground" autoFocus
              />
            </div>
            <div className="mt-6 text-center text-muted-foreground">
              <p>Search typing to search for products...</p>
            </div>
        </div>
