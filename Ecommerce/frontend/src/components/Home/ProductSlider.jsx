...
products.map(
  (product) => {
    return(
      <Link key={product._id} to={`/product/${product._id}`} className="flex shrink-0 w-80 glass-card hover:glow-on-hover animate-smooth group ">
        {/* Product Image */}
        <div className="relative overflow-hidden rounded-lg mb-4">
          ...
          <div className="absolute top-3 left-3 flex flex-col space-y-2">
                ...
                {/* Quick Add to Cart */}
                <button onClick={(e)=> handleAddToCart(product,e)} className="absolute bottom-3 right-3 p-2 glass-card hover:glow-on-hover animate-smooth opacity-0 group-hover:opacity-100 transition-opacity"
                  ...
                  <div className="flex items-center">
                    {
                      [...Array(5)].map((_,i)=>{
                        return (
                        <Star key={i} className={`w-4 h-4 ${i< Math.floor(product.ratings)
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <div>
                      <span className={`text-xs px-2 py-1 rounded ${
                        product.stock>5?"bg-green-500/20 text-gray-400":
